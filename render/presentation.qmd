  ...
  Region <- world_map$region |> unique()
    ...
    output$plot_output <- shiny::renderPlot({
      ...
      beers_dat |>
        ...
        ggplot2::theme_void() +
        ggplot2::theme(legend.position = "bottom",
                       legend.key.size = grid::unit(input$NumberBeers+2, 'cm'),
                       legend.text = ggplot2::element_text(size = (13 + input$NumberBeers)),
                       legend.title = ggplot2::element_blank(),
                       title = ggplot2::element_text(size = (17 + input$NumberBeers)))
    }
